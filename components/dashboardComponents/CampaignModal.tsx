...
const CampaignModal: React.FC<CampaignModalProps> = ({ isOpen, onClose }) => {
  ...
  const totalSteps = campaign.type === 'VOZ' ? 2 : 4

  // Efecto para saltar al paso 4 si el tipo es VOZ
  useEffect(() => {
  ...
  const nextStep = () => {
    if (currentStep < totalSteps) {
      // Si estamos en el paso 1 y el tipo es VOZ, ir directamente al paso 4
      if (currentStep === 1 && campaign.type === 'VOZ') {
        setCurrentStep(4)
      } else {
  ...
  const prevStep = () => {
    if (currentStep > 1) {
      // Si estamos en el paso 4 y el tipo es VOZ, volver al paso 1
  ...
  const calculateProgress = () => {
    if (campaign.type === 'VOZ') {
      // Para VOZ solo tenemos 2 pasos: 1 y 4
      if (currentStep === 1) {
        return 50; // 50% en el primer paso
      } else if (currentStep === 4) {
        return 100; // 100% en el último paso
      }
      return 50; // Valor por defecto (no debería llegar aquí)
    } else {
