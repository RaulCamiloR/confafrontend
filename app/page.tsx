...
import { MdOutlineCampaign, MdOutlineExplore, MdContacts, MdOutlineAnalytics, MdLogout, MdOutlineCalendarMonth, MdOutlineAssessment } from 'react-icons/md'
    ...
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-800">
      ...
      <div className="bg-white dark:bg-gray-700 p-4 shadow-md flex justify-between items-center">
        <h1 className="text-xl font-medium text-gray-800 dark:text-white">
          {`${user?.name ? 'Hola, ' : 'Cerrando sesión...'}`} <span className="font-bold text-orange-500">{user?.name || '...'}</span>
        </h1>
        <button 
          onClick={logout}
          disabled={isLoading}
        ...
        >
          {isLoading ? (
            <>
              ...
              <span className="text-gray-700 dark:text-white">Cerrando sesión...</span>
            </>
          ) : (
            <>
