  ...
  const fetchSegments = async (pageNum: number) => {
    try {
      ...
      const { data } = await axios("/api/segmentos", {
        params: { channelType, page: pageNum, pageSize, forDropdown: false },
      });
      ...
      console.log(
        `Page ${pageNum}: Received ${segmentsArray.length} items. Has next page: ${segmentsArray.length >= pageSize}`,
      );
    } catch (err) {
            ...
            {segments.length === 0 ? (
              <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-md text-center">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  No hay segmentos disponibles para el tipo de campaña{" "}
                  {campaign.type}.
                </p>
              </div>
            ) : (
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-md">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center text-gray-600 dark:text-gray-400">
              <span>
                {campaign.segment.label || campaign.segment.segmentName}
              </span>
            </div>
          </div>
        </div>
