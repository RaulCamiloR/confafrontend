...
const StepZero: React.FC<StepZeroProps> = ({ onNext }) => {
  ...
  const handleNext = () => {
    ...
    if (campaign.schedulingType === "programar" && !campaign.scheduledDate) {
      newErrors.scheduledDate =
        "Debes seleccionar una fecha para la campaña programada";
    }
    ...
    if (campaign.schedulingType === "programar" && campaign.scheduledDate) {
      ...
      if (selectedDate <= now) {
        newErrors.scheduledDate =
          "La fecha y hora programada debe ser posterior al momento actual";
      }
    }

    // validar horas en rango
    if (campaign.schedulingType === "programar" && campaign.scheduledDate) {
  ...
  const handleSchedulingTypeChange = (type: SchedulingType) => {
    updateCampaign({
      ...
      scheduledDate:
        type === "instantanea" ? new Date() : campaign.scheduledDate,
    });
    setErrors({});
  };
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selectedDate = e.target.value
      ? new Date(e.target.value + "T12:00:00.000Z")
      : null;
  ...
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newDate = campaign.scheduledDate
  ...
  const getMinTime = () => {
    if (!campaign.scheduledDate) return "07:00";
        ...
        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          {campaign.scheduledDate && (
            <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
              ...
              a las{" "}
              {new Date(campaign.scheduledDate).toLocaleTimeString("es-ES", {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </p>
